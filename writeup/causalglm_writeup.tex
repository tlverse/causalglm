...
\title{%
  \pkg{causalglm}: An \proglang{R} package for semiparametric and nonparametric generalized linear models and causal inference for heterogeneous treatment effects using targeted machine-learning
  }
\Plaintitle{causalglm: Semiparametric and nonparametric generalized linear models and causal inference for heterogeneous treatment effects using targeted machine-learning}
...
1: CATE (Intercept) 1.019485 0.04286074 0.9354792 1.10349 376.0893       0
\end{Soutput}
\begin{Sinput}
> 
\end{Sinput}
...
\begin{Soutput}
A causalglm fit object obtained from spglm for the estimand CATE with formula 
CATE(W) = 1.08 * (Intercept) + 0.998 * W
[1] "Coefficient estimates and inference:"

   type       param  tmle_est         se    lower    upper  Z_score p_value
1: CATE (Intercept) 1.0753730 0.05122491 0.974974 1.175772 331.9311       0
2: CATE           W 0.9980189 0.10467481 0.792860 1.203178 150.7532       0
\end{Soutput}
\begin{Sinput}
> 
\end{Sinput}
...
1           1  0.8689456 1.9425970 1.6340842 1.7400339 2.1451602 18.796556
2           1 -0.7512976 0.3255638 1.4996220 0.1396687 0.5114589  3.432609
3           1  0.5758009 1.6500331 1.2362676 1.4967838 1.8032824 21.103291
4           1 -0.5876185 0.4889186 1.2799929 0.3302491 0.6475882  6.039473
5           1 -0.1443844 0.9312746 0.8497541 0.8259380 1.0366113 17.328242
6           1 -0.3742521 0.7018623 1.0309905 0.5740594 0.8296652 10.763841
     p-value
1 0.0000e+00
2 5.9780e-04
3 0.0000e+00
...
\begin{Soutput}
A causalglm fit object obtained from spglm for the estimand CATE with formula 
CATE(W) = 0.904 * (Intercept) + 0.974 * W
[1] "Coefficient estimates and inference:"

   type       param  tmle_est         se     lower    upper  Z_score p_value
1: CATE (Intercept) 0.9036072 0.05254042 0.8006299 1.006585 271.9294       0
2: CATE           W 0.9743821 0.09647932 0.7852861 1.163478 159.6854       0
\end{Soutput}
\begin{Sinput}
...
\begin{Soutput}
A causalglm fit object obtained from spglm for the estimand CATE with formula 
CATE(W) = 0.906 * (Intercept) + 0.98 * W
[1] "Coefficient estimates and inference:"

   type       param  tmle_est         se     lower    upper  Z_score p_value
1: CATE (Intercept) 0.9060966 0.05244539 0.8033055 1.008888 273.1726       0
2: CATE           W 0.9803401 0.09834286 0.7875917 1.173089 157.6173       0
\end{Soutput}
\begin{Sinput}
...
\begin{Soutput}
A causalglm fit object obtained from spglm for the estimand CATE with formula 
CATE(W) = 0.918 * (Intercept) + 0.944 * W
[1] "Coefficient estimates and inference:"

   type       param  tmle_est         se     lower    upper  Z_score p_value
1: CATE (Intercept) 0.9183055 0.05866158 0.8033309 1.033280 247.5161       0
2: CATE           W 0.9441862 0.11158608 0.7254815 1.162891 133.7881       0
\end{Soutput}
\end{Schunk}
...
\begin{Soutput}
A causalglm fit object obtained from spglm for the estimand CATE with formula 
CATE(W) = 0.906 * (Intercept) + 0.967 * W
[1] "Coefficient estimates and inference:"

   type       param  tmle_est         se     lower    upper  Z_score p_value
1: CATE (Intercept) 0.9064224 0.05223572 0.8040422 1.008802 274.3677       0
2: CATE           W 0.9665985 0.09812744 0.7742723 1.158925 155.7491       0
\end{Soutput}
\end{Schunk}
...
\begin{Soutput}
A causalglm fit object obtained from spglm for the estimand CATE with formula 
CATE(W) = 0.911 * (Intercept) + 0.969 * W
[1] "Coefficient estimates and inference:"

   type       param  tmle_est         se     lower    upper  Z_score p_value
1: CATE (Intercept) 0.9108464 0.05363618 0.8057214 1.015971 268.5081       0
2: CATE           W 0.9689366 0.09913013 0.7746451 1.163228 154.5467       0
\end{Soutput}
\end{Schunk}
...
log OR(W) = 0.728 * (Intercept) + 1.3 * W
[1] "Coefficient estimates and inference:"

   type       param  tmle_est        se     lower    upper  psi_exp lower_exp
1:   OR (Intercept) 0.7278838 0.3014213 0.1371088 1.318659 2.070694  1.146953
2:   OR           W 1.3000966 0.5265851 0.2680087 2.332184 3.669651  1.307358
   upper_exp  Z_score p_value
1:  3.738404 38.18194       0
2: 10.300418 39.03705       0
\end{Soutput}
\begin{Sinput}
> # RR
...
log RR(W) = 1.04 * (Intercept) + 1.14 * W
[1] "Coefficient estimates and inference:"

   type       param tmle_est         se     lower    upper  psi_exp lower_exp
1:   RR (Intercept) 1.038312 0.09384462 0.8543801 1.222244 2.824446  2.349917
2:   RR           W 1.139956 0.18255461 0.7821558 1.497757 3.126631  2.186180
   upper_exp   Z_score p_value
1:  3.394798 174.93978       0
2:  4.471646  98.73369       0
\end{Soutput}
\end{Schunk}
...
CATE(W) = 0.939 * (Intercept) + 1.01 * W
[1] "Coefficient estimates and inference:"

   type       param  tmle_est        se     lower    upper   Z_score p_value
1: CATE (Intercept) 0.9389794 0.1076974 0.7278963 1.150062 137.85442       0
2: CATE           W 1.0134605 0.1800344 0.6605995 1.366321  89.00641       0
\end{Soutput}
\begin{Sinput}
> # CATT
> Y <- rnorm(n, mean = A + A * W + W + sin(5 * W), sd = 0.5)
...
CATT(W) = 0.976 * (Intercept) + 0.965 * W
[1] "Coefficient estimates and inference:"

   type       param  tmle_est         se     lower    upper  Z_score p_value
1: CATT (Intercept) 0.9758269 0.05833516 0.8614921 1.090162 264.4919       0
2: CATT           W 0.9652482 0.09224862 0.7844442 1.146052 165.4433       0
\end{Soutput}
\begin{Sinput}
> # TSM
> Y <- rnorm(n, mean = A + A * W + W + sin(5 * W), sd = 0.5)
...
TSM(W) = 0.0148 * E[Y_{A=0}]: (Intercept) + 0.517 * E[Y_{A=0}]: W + 0.982 * E[Y_{A=1}]: (Intercept) + 1.73 * E[Y_{A=1}]: W
[1] "Coefficient estimates and inference:"

   type                   param   tmle_est         se      lower     upper
1:  TSM E[Y_{A=0}]: (Intercept) 0.01475944 0.05938714 -0.1016372 0.1311561
2:  TSM           E[Y_{A=0}]: W 0.51688973 0.10003385  0.3208270 0.7129525
3:  TSM E[Y_{A=1}]: (Intercept) 0.98159072 0.06068563  0.8626491 1.1005324
4:  TSM           E[Y_{A=1}]: W 1.72804523 0.10517170  1.5219125 1.9341780
      Z_score   p_value
1:   3.929593 8.509e-05
2:  81.699789 0.000e+00
3: 255.749389 0.000e+00
4: 259.792270 0.000e+00
\end{Soutput}
\begin{Sinput}
> # OR
> Y <- rbinom(n, size = 1, prob = plogis(A + A * W + W + sin(5 * W)))
...
log OR(W) = 0.966 * (Intercept) + 1.49 * W
[1] "Coefficient estimates and inference:"

   type       param  tmle_est        se     lower    upper  psi_exp lower_exp
1:   OR (Intercept) 0.9662105 0.3113014 0.3560709 1.576350 2.627967  1.427709
2:   OR           W 1.4881087 0.6193364 0.2742316 2.701986 4.428712  1.315519
   upper_exp  Z_score p_value
1:  4.837268 49.07503       0
2: 14.909310 37.99076       0
\end{Soutput}
\begin{Sinput}
> # RR
> Y <- rpois(n, lambda = exp(A + A * W + sin(5 * W)))
...
log RR(W) = 1.12 * (Intercept) + 0.809 * W
[1] "Coefficient estimates and inference:"

   type       param  tmle_est        se     lower    upper  psi_exp lower_exp
1:   RR (Intercept) 1.1206114 0.1019987 0.9206976 1.320525 3.066728  2.511041
2:   RR           W 0.8091963 0.1774296 0.4614407 1.156952 2.246102  1.586358
   upper_exp   Z_score p_value
1:  3.745388 173.71224       0
2:  3.180225  72.11039       0
\end{Soutput}
\begin{Sinput}
> head(predict(output, data = data))
\end{Sinput}
...
1           1 -0.55729312 1.953557 2.192770 1.488596 2.563748  4.828653
2           1  0.06640036 3.236014 1.632138 2.643279 3.961664 11.376479
3           1 -0.48735734 2.067300 2.064701 1.600475 2.670289  5.561541
4           1  0.78650221 5.795293 2.793547 4.099046 8.193471  9.944826
5           1 -0.31014927 2.386051 1.796132 1.909782 2.981095  7.655457
6           1 -0.68688421 1.759073 2.453994 1.297691 2.384495  3.638994
     p-value
1 1.3746e-06
2 0.0000e+00
3 2.6740e-08
