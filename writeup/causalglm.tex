...
CATE(W) = 1.03 * (Intercept)

Coefficient estimates and inference:
   type       param tmle_est         se    lower    upper  Z_score p_value
1: CATE (Intercept) 1.032767 0.03918245 0.955971 1.109563 416.7551       0
\end{Soutput}
\begin{Sinput}
> 
\end{Sinput}
...
\begin{Soutput}
A causalglm fit object obtained from spglm for the estimand CATE with formula: 
CATE(W) = 0.975 * (Intercept) + 0.96 * W

Coefficient estimates and inference:
   type       param  tmle_est         se     lower    upper  Z_score p_value
1: CATE (Intercept) 0.9745196 0.05031080 0.8759122 1.073127 306.2664       0
2: CATE           W 0.9600299 0.08403521 0.7953239 1.124736 180.6315       0
\end{Soutput}
\begin{Sinput}
> 
\end{Sinput}
...
1           1  0.6674362 1.6152782 1.2006050  1.466449702 1.7641068 21.272435
2           1  0.9888739 1.9238681 1.5466351  1.732145193 2.1155909 19.667874
3           1  0.6118783 1.5619410 1.1463517  1.419837791 1.7040442 21.543525
4           1  0.4255187 1.3830302 0.9831626  1.261156110 1.5049043 22.242128
5           1 -0.8419996 0.1661748 1.3625207 -0.002725041 0.3350746  1.928377
6           1  0.1067398 1.0769930 0.8102206  0.976557000 1.1774290 21.017429
   p-value
1 0.000000
2 0.000000
3 0.000000
...
\begin{Soutput}
A causalglm fit object obtained from spglm for the estimand CATE with formula: 
CATE(W) = 0.98 * (Intercept) + 0.974 * W

Coefficient estimates and inference:
   type       param  tmle_est         se     lower    upper  Z_score p_value
1: CATE (Intercept) 0.9802002 0.05549227 0.8714374 1.088963 279.2880       0
2: CATE           W 0.9736086 0.09960716 0.7783822 1.168835 154.5482       0
\end{Soutput}
\begin{Sinput}
...
\begin{Soutput}
A causalglm fit object obtained from spglm for the estimand CATE with formula: 
CATE(W) = 0.98 * (Intercept) + 0.976 * W

Coefficient estimates and inference:
   type       param  tmle_est         se     lower    upper  Z_score p_value
1: CATE (Intercept) 0.9795675 0.05461465 0.8725247 1.086610 283.5928       0
2: CATE           W 0.9759605 0.09555900 0.7886683 1.163253 161.4844       0
\end{Soutput}
\begin{Sinput}
...
\begin{Soutput}
A causalglm fit object obtained from spglm for the estimand CATE with formula: 
CATE(W) = 0.954 * (Intercept) + 0.972 * W

Coefficient estimates and inference:
   type       param  tmle_est         se     lower    upper  Z_score p_value
1: CATE (Intercept) 0.9544447 0.06345007 0.8300848 1.078805 237.8420       0
2: CATE           W 0.9721127 0.11664391 0.7434949 1.200731 131.7724       0
\end{Soutput}
\end{Schunk}
...
\begin{Soutput}
A causalglm fit object obtained from spglm for the estimand CATE with formula: 
CATE(W) = 0.977 * (Intercept) + 0.975 * W

Coefficient estimates and inference:
   type       param  tmle_est         se     lower    upper  Z_score p_value
1: CATE (Intercept) 0.9773086 0.05561507 0.8683050 1.086312 277.8492       0
2: CATE           W 0.9751120 0.10161903 0.7759424 1.174282 151.7223       0
\end{Soutput}
\end{Schunk}
...
\begin{Soutput}
A causalglm fit object obtained from spglm for the estimand CATE with formula: 
CATE(W) = 0.981 * (Intercept) + 0.975 * W

Coefficient estimates and inference:
   type       param  tmle_est         se     lower    upper  Z_score p_value
1: CATE (Intercept) 0.9805243 0.05688198 0.8690376 1.092011 272.5547       0
2: CATE           W 0.9747895 0.09928161 0.7802011 1.169378 155.2430       0
\end{Soutput}
\end{Schunk}
...
log OR(W) = 0.392 * (Intercept) + 0.109 * W

Coefficient estimates and inference:
   type       param  tmle_est        se      lower    upper  psi_exp lower_exp
1:   OR (Intercept) 0.3915748 0.3104984 -0.2169909 1.000141 1.479309 0.8049373
2:   OR           W 0.1094648 0.5586958 -0.9855589 1.204488 1.115681 0.3732306
   upper_exp   Z_score   p_value
1:  2.718664 19.940011 0.0000000
2:  3.335053  3.097912 0.0019489
\end{Soutput}
\begin{Sinput}
> # RR
...
log RR(W) = 0.917 * (Intercept) + 0.894 * W

Coefficient estimates and inference:
   type       param  tmle_est         se     lower    upper  psi_exp lower_exp
1:   RR (Intercept) 0.9165544 0.09203699 0.7361652 1.096944 2.500659  2.087913
2:   RR           W 0.8938860 0.20276123 0.4964813 1.291291 2.444611  1.642930
   upper_exp   Z_score p_value
1:  2.994998 157.45841       0
2:  3.637479  69.70553       0
\end{Soutput}
\end{Schunk}
...
CATE(W) = 1.1 * (Intercept) + 1 * W

Coefficient estimates and inference:
   type       param tmle_est        se     lower    upper   Z_score p_value
1: CATE (Intercept) 1.097678 0.1146811 0.8729068 1.322448 151.33977       0
2: CATE           W 1.002677 0.1926442 0.6251010 1.380252  82.29529       0
\end{Soutput}
\begin{Sinput}
> # CATT
> Y <- rnorm(n, mean = A + A * W + W + sin(5 * W), sd = 0.5)
...
CATT(W) = 0.952 * (Intercept) + 1.17 * W

Coefficient estimates and inference:
   type       param tmle_est         se     lower    upper  Z_score p_value
1: CATT (Intercept) 0.951993 0.06477445 0.8250374 1.078949 232.3807       0
2: CATT           W 1.166143 0.10687042 0.9566805 1.375605 172.5298       0
\end{Soutput}
\begin{Sinput}
> # TSM
> Y <- rnorm(n, mean = A + A * W + W + sin(5 * W), sd = 0.5)
...
TSM(W) = -0.0494 * E[Y_{A=0}]: (Intercept) + 0.764 * E[Y_{A=0}]: W + 1.1 * E[Y_{A=1}]: (Intercept) + 1.69 * E[Y_{A=1}]: W

Coefficient estimates and inference:
   type                   param    tmle_est         se      lower      upper
1:  TSM E[Y_{A=0}]: (Intercept) -0.04935325 0.05955152 -0.1660721 0.06736559
2:  TSM           E[Y_{A=0}]: W  0.76387657 0.10015094  0.5675843 0.96016880
3:  TSM E[Y_{A=1}]: (Intercept)  1.10200971 0.06513269  0.9743520 1.22966744
4:  TSM           E[Y_{A=1}]: W  1.69228555 0.11611996  1.4646946 1.91987649
     Z_score p_value
1:  13.10367       0
2: 120.59746       0
3: 267.52009       0
...
log OR(W) = 1.66 * (Intercept) + 2.24 * W

Coefficient estimates and inference:
   type       param tmle_est        se     lower    upper  psi_exp lower_exp
1:   OR (Intercept) 1.661097 0.4083402 0.8607646 2.461429 5.265082  2.364968
2:   OR           W 2.238777 0.8144713 0.6424424 3.835111 9.381849  1.901119
   upper_exp  Z_score p_value
1:  11.72155 64.31951       0
2:  46.29858 43.46153       0
\end{Soutput}
\begin{Sinput}
> # RR
> Y <- rpois(n, lambda = exp(A + A * W + sin(5 * W)))
...
log RR(W) = 1.08 * (Intercept) + 0.973 * W

Coefficient estimates and inference:
   type       param  tmle_est        se     lower    upper  psi_exp lower_exp
1:   RR (Intercept) 1.0825615 0.1135637 0.8599807 1.305142 2.952232  2.363115
2:   RR           W 0.9731292 0.2436707 0.4955433 1.450715 2.646212  1.641390
   upper_exp   Z_score p_value
1:  3.688214 150.72417       0
2:  4.266164  63.14474       0
\end{Soutput}
\begin{Sinput}
> head(predict(output, data = data))
\end{Sinput}
...
1           1  0.1799870 3.517365 1.756350 2.829197 4.372922 11.322446
2           1  0.6617003 5.620842 2.728206 4.007988 7.882723 10.005867
3           1 -0.2390070 2.339593 2.212649 1.778364 3.077937  6.073858
4           1  0.3346310 4.088592 1.932130 3.217779 5.195070 11.523864
5           1  0.3077931 3.983194 1.889363 3.151493 5.034387 11.566157
6           1 -0.1106781 2.650789 1.946135 2.082589 3.374013  7.920236
     p-value
1 0.0000e+00
2 0.0000e+00
3 1.2487e-09
