...
nsims <- 1000 # There is quite a lot of randomness in coverage. Large sim numbers gives more correct coverage probabilities
learning_method <- "HAL" # Recommended machine-learning algorithm (barring computational constraints)
```
...
for(i in 1:nsims){
  ...
  out <- causalGLMsp(formula, data, W, A, Y, estimand = "CATE", learning_method = learning_method, verbose = T, HAL_fit_control = list(parallel = TRUE))
  passes <- cbind(passes, out$coefs$lower <= beta0 & beta0 <= out$coefs$upper )
   
  out <- causalGLMnp(formula, data, W, A, Y, estimand = "CATE", learning_method = learning_method, verbose = T, HAL_fit_control = list(parallel = TRUE))
    passes1 <- cbind(passes1, out$coefs$lower <= beta0 & beta0 <= out$coefs$upper )

 
   out <- causalGLMnp(formula, data, W, A, Y, estimand = "CATT", learning_method = learning_method, verbose = T, HAL_fit_control = list(parallel = TRUE))
...
for(i in 1:nsims){
  ...
  out <- causalGLMsp(formula, data, W, A, Y, estimand = "OR", learning_method = learning_method, verbose = FALSE, delta_epsilon = 0.05, HAL_fit_control = list(parallel = TRUE))
 
  passes <- cbind(passes, out$coefs$lower <= beta0 & beta0 <= out$coefs$upper )
  print(rowMeans(passes), HAL_fit_control = list(parallel = TRUE))
  
    out <- causalGLMnp(formula, data, W, A, Y, estimand = "OR", learning_method = learning_method, verbose = FALSE, delta_epsilon = 0.05, HAL_fit_control = list(parallel = TRUE))
...
for(i in 1:nsims){
  ...
  out <- causalGLMsp(formula, data, W, A, Y, estimand = "RR", learning_method = learning_method, verbose = FALSE, HAL_fit_control = list(parallel = TRUE))
  
  passes <- cbind(passes, out$coefs$lower <= beta0 & beta0 <= out$coefs$upper )
  
   out <- causalGLMnp(formula, data, W, A, Y, estimand = "RR", learning_method = learning_method, verbose = FALSE, HAL_fit_control = list(parallel = TRUE))
