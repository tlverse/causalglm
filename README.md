...
Notably, if formula = ~1 is passed to causalRobustGLM then the coefficient is an efficient nonparametric estimator of the ATE, which may be of independent interest. In particular, if V = 1 (i.e. formula = ~1) then the solution is equal to beta := E[CATE(W)] = E[E[Y|A=1,W] - E[Y|A=0,W]] = ATE, which is exactly the average treatment effect. Thus, this nonparametric working-model-based estimator does capture nonparametric causal additive treatment effects.
