...
for(i in 1:nsims){
  ...
  out <- causalGLMsp(formula, data, W, A, Y, estimand = "CATE", learning_method = "glmnet", verbose = T, HAL_fit_control = list(parallel = TRUE))
  passes <- cbind(passes, out$coefs$lower <= beta0 & beta0 <= out$coefs$upper )
  print(out$coefs)
  out <- causalGLMnp(formula, data, W, A, Y, estimand = "CATE", learning_method = "glmnet", verbose = T, HAL_fit_control = list(parallel = TRUE))
    passes1 <- cbind(passes1, out$coefs$lower <= beta0 & beta0 <= out$coefs$upper )

    print(out$coefs)
   out <- causalGLMnp(formula, data, W, A, Y, estimand = "CATT", learning_method = "glmnet", verbose = T, HAL_fit_control = list(parallel = TRUE))
    print(out$coefs)
...
for(i in 1:nsims){
  ...
  out <- causalGLMsp(formula, data, W, A, Y, estimand = "OR", learning_method = "glm", verbose = FALSE, delta_epsilon = 0.05)
 
  passes <- cbind(passes, out$coefs$lower <= beta0 & beta0 <= out$coefs$upper )
  print(rowMeans(passes))
  
    out <- causalGLMnp(formula, data, W, A, Y, estimand = "OR", learning_method = "glm", verbose = FALSE, delta_epsilon = 0.05)
...
for(i in 1:nsims){
  ...
  out <- causalGLMsp(formula, data, W, A, Y, estimand = "RR", learning_method = "glm", verbose = FALSE)
  
  passes <- cbind(passes, out$coefs$lower <= beta0 & beta0 <= out$coefs$upper )
  
   out <- causalGLMnp(formula, data, W, A, Y, estimand = "RR", learning_method = "glm", verbose = FALSE)
