...
#' @export 
causalGLM <- function(formula, data, W, A, Y, estimand = c("CATE", "OR", "RR"),   learning_method = c(  "HAL", "SuperLearner", "glm", "glmnet", "gam", "mars", "ranger", "xgboost"),    append_interaction_matrix = TRUE,  cross_fit = FALSE,  sl3_Learner_A = NULL, sl3_Learner_Y = NULL, wrap_in_Lrnr_glm_sp = TRUE,   weights = NULL,   HAL_args_Y0W = list(smoothness_orders = 1, max_degree = 1, num_knots = c(10,5,1)), HAL_fit_control = list(parallel = F), sl3_Learner_var_Y = Lrnr_glmnet$new(family = "poisson") , ... ){
  ...
  tmle_spec_sp <- tmle3_Spec_spCausalGLM$new(formula = formula, estimand = estimand, append_interaction_matrix = append_interaction_matrix, wrap_in_Lrnr_glm_sp = wrap_in_Lrnr_glm_sp , binary_outcome = binary_outcome )
  ...
  if(is.null(sl3_Learner_Y)) {
    if(learning_method == "HAL") {
      ...
      superlearner_default <- make_learner(Pipeline, Lrnr_cv$new(list(  Lrnr_glmnet$new(formula = formula_Y),  Lrnr_glm$new(formula = formula_Y), Lrnr_gam$new(formula = formula_Y), Lrnr_earth$new(formula = formula_Y) ,
                                                                        Lrnr_ranger$new() ,  Lrnr_xgboost$new(max_depth =3 ), Lrnr_xgboost$new(max_depth =4 ), Lrnr_xgboost$new(max_depth =5) ) ), Lrnr_cv_selector$new(loss_function_least_squares))
      learner_list_Y = list(SuperLearner = superlearner_default, glmnet =  Lrnr_glmnet$new(formula = formula_Y), glm = Lrnr_glm$new(formula = formula_Y), gam = Lrnr_gam$new(formula = formula_Y), mars = Lrnr_earth$new(formula = formula_Y) ,
                              ranger = Lrnr_cv$new(Lrnr_ranger$new()), xgboost = make_learner(Pipeline, Lrnr_cv$new( Stack$new(  Lrnr_xgboost$new(max_depth =3 ), Lrnr_xgboost$new(max_depth =4 ), Lrnr_xgboost$new(max_depth =5) )), Lrnr_cv_selector$new(loss_squared_error)))
...
superlearner_RR <- make_learner(Pipeline, Lrnr_cv$new(list(  Lrnr_glmnet$new(family = "poisson"),  Lrnr_glm$new(family = poisson()), Lrnr_gam$new(family = poisson()),  
                                                                  Lrnr_xgboost$new(max_depth =3 , objective = "count:poisson"), Lrnr_xgboost$new(max_depth =4, objective = "count:poisson" ), Lrnr_xgboost$new(max_depth =5,  objective = "count:poisson") ) ), Lrnr_cv_selector$new(loss_function_least_squares))
...
learner_list_A = list(HAL = Lrnr_hal9001$new(max_degree = 2, smoothness_orders  = 1, num_knots = c(10,3)), SuperLearner = superlearner_default, glmnet =  Lrnr_glmnet$new(), glm = Lrnr_glm$new(), gam = Lrnr_gam$new(), mars = Lrnr_earth$new() ,
                        ranger = Lrnr_cv$new(Lrnr_ranger$new()), xgboost = make_learner(Pipeline, Lrnr_cv$new( Stack$new(  Lrnr_xgboost$new(max_depth =3 ), Lrnr_xgboost$new(max_depth =4 ), Lrnr_xgboost$new(max_depth =5) )), Lrnr_cv_selector$new(loss_loglik_binomial)))
...
learner_list_Y0W_RR = list(SuperLearner = superlearner_RR, glmnet =  Lrnr_glmnet$new(family = "poisson"), glm = Lrnr_glm$new(family = poisson()), gam = Lrnr_gam$new(family = poisson()), 
                        xgboost = make_learner(Pipeline, Lrnr_cv$new( Stack$new(  Lrnr_xgboost$new(max_depth =3,  objective = "count:poisson", eval_metric = "error"), Lrnr_xgboost$new(max_depth =4, objective = "count:poisson", eval_metric = "error"), Lrnr_xgboost$new(max_depth =5, objective = "count:poisson", eval_metric = "error") )), Lrnr_cv_selector$new(loss_squared_error)))
