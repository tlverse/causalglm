...
for(i in 1:nsims){
  ...
  out <- spglm(formula, data, W, A, Y, estimand = "OR", learning_method = learning_method, verbose = FALSE, delta_epsilon = 0.05, HAL_fit_control = list(parallel = TRUE))
 
  passes <- cbind(passes, out$coefs$lower <= beta0 & beta0 <= out$coefs$upper )
  print(rowMeans(passes), HAL_fit_control = list(parallel = TRUE))
  
    out <- npglm(formula, data, W, A, Y, estimand = "OR", learning_method = learning_method, verbose = FALSE, delta_epsilon = 0.05, HAL_fit_control = list(parallel = TRUE))
